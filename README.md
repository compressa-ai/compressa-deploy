...
compressa/aim-ui:0.3.10
compressa/aim-server:0.3.10
nginx:latest
...
1. Change access: `chmod -R 777 deploy/platform/build`, `chmod -R 777 deploy/platform/test_results`
2. Create common Docker network: `docker network create test_network`
3. Edit the config: `deploy/platform/build/config.yaml`
...
It is possible to edit Platforms `docker-compose.yaml` and `.env` files to run Layout Model on GPU
